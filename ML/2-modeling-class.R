# The following script is for training and testing Classification models
...
ds_train <- ds_train %>% dplyr::select(-FT4)
ds_test  <- ds_test %>% dplyr::select(-FT4)
...
# Neural Net, KNN
normalized_rec <- recipes::recipe(ft4_dia ~ ., data = ds_train) %>%
...
rf_rec <- recipes::recipe(ft4_dia ~ . , data = ds_train) %>%
  recipes::step_impute_bag(recipes::all_predictors())

boost_rec <- recipes::recipe(ft4_dia ~ . , data = ds_train) %>%
  recipes::step_impute_bag(recipes::all_predictors()) %>%
  recipes::step_dummy(gender)
